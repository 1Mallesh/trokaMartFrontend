...
export default function CategoriesPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {categories.map((category) => (
            <Link
              key={category.name}
              href={category.href}
              className="bg-white rounded-xl shadow-md p-5 sm:p-6 hover:shadow-lg transition-all duration-200 group h-full"
            >
              <div className="text-center">
                <div className="text-5xl sm:text-6xl mb-4 group-hover:scale-110 transition-transform">
                  {category.icon}
                </div>
                <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2 group-hover:text-green-600">
                  {category.name}
                </h3>
      ...
      {/* Call to Action */}
      <div className="bg-green-600 text-white py-10 sm:py-12">
